...
fn create_cli_snapshot() {
  let Ok(snapshot_name) = env::var("SNAPSHOT_NAME") else {
    panic!("No SNAPSHOT_NAME provided");
  };
  ...
  let snapshot_dir = if cfg!(debug_assertions) {
    manifest_dir.join("snapshots").join("debug")
  } else {
    manifest_dir.join("snapshots").join(env::var("PROFILE").unwrap())
  };
